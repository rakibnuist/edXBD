  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (response.ok) {
        ...
        trackDatabaseOperation('fetch_dashboard_data', false, { error: 'api_error' });
      }
    } catch (error) {
      ...
      trackDatabaseOperation('fetch_dashboard_data', false, { error: 'network_error' });
    } finally {
      setLoading(false);
    }
  };
              ...
              {stats.recentLeads.map((lead) => (
                <div key={lead._id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">{lead.country} - {lead.course}</p>
                  </div>
